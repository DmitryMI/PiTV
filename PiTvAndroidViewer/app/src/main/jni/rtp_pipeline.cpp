...
{
    JNIEnv *env = static_cast<JNIEnv*>(pthread_getspecific(current_jni_env));
...
static void gst_native_init (JNIEnv * env, jobject thiz)
{
    ...
    GST_DEBUG_CATEGORY_INIT (debug_category, "rtp_pipeline", 0, "PiTv RTP pipeline");
    gst_debug_set_threshold_for_name ("rtp_pipeline", GST_LEVEL_DEBUG);
...
static void gst_native_set_udp_port(JNIEnv * env, jobject thiz, int port)
{
    ...
    if (!data)
        return;
    GST_DEBUG ("Changing UDP port to %d", port);
    GstElement* udpsrc = gst_bin_get_by_name(GST_BIN(data->pipeline), "udpsrc");
    if(!udpsrc)
    {
        GST_DEBUG ("udpsrc not found!");
        return;
    }
...
static jboolean gst_native_class_init (JNIEnv * env, jclass klass)
{
    ...
    if (!custom_data_field_id || !set_message_method_id
         ...
         */
        __android_log_print (ANDROID_LOG_ERROR, "rtp_pipeline",
...
static JNINativeMethod native_methods[] = {
        ...
        {"nativeSetUdpPort", "(I)V", (void*) gst_native_set_udp_port}
};
...
jint JNI_OnLoad (JavaVM * vm, void *reserved)
{
    ...
    if ((vm)->GetEnv ((void **) &env, JNI_VERSION_1_4) != JNI_OK) {
        __android_log_print (ANDROID_LOG_ERROR, "rtp_pipeline", "Could not retrieve JNIEnv");
        return 0;
    }
    jclass klass = (env)->FindClass ("com/dmitriymi/pitvandroidviewer/CameraViewActivity");
